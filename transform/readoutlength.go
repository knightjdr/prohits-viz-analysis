...
func ReadoutLength(data []map[string]string, readoutColumn string) (transformed []map[string]string) {
	transformed = data
	...
	for _, row := range transformed {
		...
		if _, ok := readouts[readoutName]; !ok {
			length, _ := strconv.Atoi(row["readoutLength"])
	...
	median := helper.MedianInt(readoutLengths)

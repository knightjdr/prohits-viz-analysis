...
func addAbundance(data map[string]map[string]map[string]float64, analysis *types.Analysis) {
	...
	for _, row := range analysis.Data {
		...
		if doesScorePassFilter(score) && abundance >= analysis.Settings.MinAbundance {

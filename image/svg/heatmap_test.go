...
func TestHeatmap(t *testing.T) {
	...
	matrices.Conditions = []string{"bait1", "bait2", "bait3"}
	matrices.Ratio = [][]float64{
		{1, 0.5, 0.3},
		{1, 0.3, 0.1},
		{0.5, 0.25, 1},
	}
	matrices.Readouts = []string{"prey1", "prey2", "prey3"}
	...
	data := new(Data)
	data.Annotations = typedef.Annotations{
		FontSize: 15,
	...
	assert.Equal(t, "", svg, "Heatmap return value should be null when writing to file")
	assert.Equal(t, want, string(bytes), "Heatmap svg is not correct when writing to file")
}

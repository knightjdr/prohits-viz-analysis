...
func TestHeatmap(t *testing.T) {
	...
	want := "{\n\t\"columns\": {\n" +
		...
		"\t\t\t\t\"name\": \"row3\"\n\t\t\t}\n\t\t]\n\t},\n" +
		"\t\"minimap\": {\n\t\t\"image\": \"pngImage\"\n\t}\n}\n"
	Heatmap(columns, data, parameters, settings, uri, "test/interactive.json")
	bytes, _ := afero.ReadFile(fs.Instance, "test/interactive.json")
	assert.Equal(t, want, string(bytes), "Heatmap json is not correct")
}

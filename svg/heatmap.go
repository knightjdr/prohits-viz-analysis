...
func Heatmap(matrix [][]float64, columns, rows []string, options map[string]interface{}) (svg string) {
	...
	for i, colName := range columns {
		...
		text := fmt.Sprintf(
			"\t\t<text y=\"%d\" x=\"%d\" font-family=\"Helvetica\" font-size=\"%d\""+
	...
	// Get color gradient.
	colorGradient := ColorGradient(options["colorSpace"].(string), 101)
